...
```
mabatisplus中已经引入了mybatis依赖不需要再额外引入
## 配置数据库信息
...
```java
 QueryWrapper<User> queryWrapper = new QueryWrapper<>( );
//        QueryWrapper<User> queryWrapper = new QueryWrapper<>(new User());
//        queryWrapper.select("name","user_name");
...
public class UserServiceImpl extends ServiceImpl<UserMapper,User> implements UserService {
    ...
    @Autowired
    private OrdersMapper ordersMapper;//注入其他mapper

    @Override
    public User test() {
        UserMapper userMapper = getBaseMapper();//获取继承的mapper
