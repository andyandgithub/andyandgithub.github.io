...
## 新点
### 子模块，父模块
maven 父项目工程之下建立几个子模块maven项目，抽出公用模块作为公用模块项目利用
```xml
...
```
父模块中的管理不是真的引入，而是在子模块真正引入的时候，规定版本号的作用，子模块就无需增加版本号了。

在非公共模块中引入公共模块利用maven的传递依赖性

若是更改了代码运行失败可以重新部署一下各个模块的maven
```xml
<dependency>
    <groupId>com.andy</groupId>
    <artifactId>andy-framework</artifactId>
    <version>1.0-SNAPSHOT</version>
</dependency>
```
## easycode 插件生成
由表生成实体类和`mappper`，`controller`，`service`，`serviceimpl`

## 返回结果封装
返回状态码code，返回信息msg，返回数据 data
## 字段常量
使用枚举类型或者字段常量
## 跨域问题
使用
...
原有字段过于杂乱。查询结果不适合将所有查询结果统一发送到前端因此选出部分字段作为发送的示例

vo类要发送的字段保留下来，类型和命名要与原类一致，方便拷贝。

## bean拷贝
使用反射和泛型实现bean示例的拷贝和bean列表的拷贝工具类 `BeanCopyUtils`
## @Accessors(chain = true)
lombok 链式为set方法增加返回值为本对象

## 获取用户id
将通过token获取用户id的方法封装，以后遇到使用token进行访问的时直接获取该id的信息
## 自定义异常
自定义异常并且，全局捕获
```java
@Data
...
## 配置mybatisplus的自动填充
```java
package com.andy.handler.mybatisplus;

import com.andy.utils.SecurityUtils;
...
```
## oss服务
用户上传图片等资源存储在服务器会占据服务器资源，提高服务器的开销，推荐使用oss服务器专门存储上传的资源。

## 使用redis统计浏览量，减少数据库的访问
开机启动应用时候开始执行，使用corn定时表达式定时执行某项任务
```java
...
## 后台权限校验
防止有token的用户访问不属于该用户的权限1
## Long类型转化为String类型返回
Long很长时可能会超过前端js的Long范围而发生精度丢失，所以需要转化

## 事务处理
`@Transactional`

添加注解，add文章的时候，insert文章内容和文章-tag关联表必须都完成或者都不完成，是一个事务
...
我们有前台和后台两套系统，但是实际上两套后端系统的很多内容是可能重复的。这里如果我们只是单纯的创建两个后端工程。那么就会有大量的重复代码，并且需要修改的时候也需要修改两次。这就是代码复用性不高。
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <groupId>com.andy</groupId>
    <artifactId>AndyBlog</artifactId>
    <packaging>pom</packaging>
    <version>1.0-SNAPSHOT</version>

[//]: # 模块的聚合(    )
    <modules>
        <module>andy-framework</module>
        <module>andy-admin</module>
        <module>andy-blog</module>
    </modules>
...
②创建公共子模块 `andy-framework`

父工程里面写入了版本控制和依赖的管理，因此子模块无需再次写版本号，默认使用父模块规定的版本
```xml
<?xml version="1.0" encoding="UTF-8"?>
<project xmlns="http://maven.apache.org/POM/4.0.0"
         xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <artifactId>andy-admin</artifactId>
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <artifactId>andy-blog</artifactId>
...
public class andyBlogApplication {
...
spring:
  datasource:
    url: jdbc:mysql://localhost:3306/andy_blog?characterEncoding=utf-8&serverTimezone=Asia/Shanghai
    username: root
    password: root
...
我们可以暂时先注释掉andy-framework中的SpringSecurity依赖方便测试
...
import com.andy.enums.AppHttpCodeEnum;
...
public class ArticleServiceImpl extends ServiceImpl<ArticleMapper, Article> implements ArticleService {
    ...
    public ResponseResult hotArticleList() {
        ...
        //必须是正式文章也可以写列名，编译的时候不会出错，建议使用下列方式
...
#### 3.2.0 分类表分析
		...
		建表SQL及初始化数据见：AndyBlog\资源\SQL\andy_category.sql
...
建表SQL及初始化数据见：AndyBlog\资源\SQL\andy_link.sql	
...
RedisCache redis的存储和读取

SecurityUtil 用于获取当前token的用户id
...
建表SQL及初始化数据见：`AndyBlog\资源\SQL\andy_comment.sql`	
...
```json
{
    "avatar":"https://andy-blog-oss.oss-cn-beijing.aliyuncs.com/2022/01/31/948597e164614902ab1662ba8452e106.png",
    "email":"23412332@qq.com",
    "id":"1",
    "nickName":"andy333",
    "sex":"1"
...
如果希望在SpringBoot应用启动时进行一些初始化操作可以选择使用`CommandLineRunner`来进行处理。

我们只需要实现`CommandLineRunner`接口，并且把对应的bean注入容器。把相关初始化的代码重新到需要重新的方法中。
...
定时任务的实现方式有很多，比如`XXL-Job`等。但是其实核心功能和概念都是类似的，很多情况下只是调用的API不同而已。
...
spring:
  datasource:
    url: jdbc:mysql://localhost:3306/andy_blog?characterEncoding=utf-8&serverTimezone=UTC
    username: root
    password: root
    driver-class-name: com.mysql.cj.jdbc.Driver
  servlet:
...
③ SQL语句
	...
	SQL脚本：AndyBlog\资源\SQL\andy_tag.sql
...
#### 表分析
	...
	建表SQL及初始化数据见：AndyBlog\资源\SQL\andy_menu.sql
...
<?xml version="1.0" encoding="UTF-8" ?>
<!DOCTYPE mapper PUBLIC "-//mybatis.org//DTD Mapper 3.0//EN" "http://mybatis.org/dtd/mybatis-3-mapper.dtd" >
<mapper namespace="com.andy.mapper.RoleMapper">
...
```xml
 <select id="selectAllRouterMenu" resultType="com.andy.domain.entity.Menu">
        SELECT
    ...
    <select id="selectRouterMenuTreeByUserId" resultType="com.andy.domain.entity.Menu">
        SELECT
          DISTINCT m.id, m.parent_id, m.menu_name, m.path, m.component, m.visible, m.status, IFNULL(m.perms,'') AS perms, m.is_frame,  m.menu_type, m.icon, m.order_num, m.create_time
        FROM
            `sys_user_role` ur
...
SQL脚本：AndyBlog\资源\SQL\andy_article_tag.sql
...
{
    "title":"测试新增博文",
    "thumbnail":"https://andy-blog-oss.oss-cn-beijing.aliyuncs.com/2022/08/21/4ceebc07e7484beba732f12b0d2c43a9.png",
    "isTop":"0",
    "isComment":"0",
    "content":"# 一级标题\n## 二级标题\n![Snipaste_20220228_224837.png](https://andy-blog-oss.oss-cn-beijing.aliyuncs.com/2022/08/21/c3af554d4a0f4935b4073533a4c26ee8.png)\n正文",
    "tags":[
        1,
        4
    ],
...
在andy-admin中增加UploadController
...
```java
@TableName(value="andy_article_tag")
@Data
@AllArgsConstructor
@NoArgsConstructor
...
andy   eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiJkZGJkNjM5MWJiZTA0NmMzOTc4NDg1ZTcxNWQ3YjQ0MSIsInN1YiI6IjEiLCJpc3MiOiJzZyIsImlhdCI6MTY2MjI0NDE4NywiZXhwIjoxNjYyMzMwNTg3fQ.z4JGwFN3lWyVbOCbhikCe-O4D6SvCQFEE5eQY3jDJkw
		...
		"rows":[
			{
				...
				"thumbnail":"https://andy-blog-oss.oss-cn-beijing.aliyuncs.com/2022/01/31/948597e164614902ab1662ba8452e106.png",
				"title":"SpringSecurity从入门到精通",
				"viewCount":"161"
			}
		],
...
```json
{
	...
	"data":{
		...
		"thumbnail":"https://andy-blog-oss.oss-cn-beijing.aliyuncs.com/2022/08/28/7659aac2b74247fe8ebd9e054b916dbf.png",
		"title":"委屈饿驱蚊器",
		"updateBy":"1",
...
{
    "categoryId":"1",
    "content":"![Snipaste_20220228_224837.png](https://andy-blog-oss.oss-cn-beijing.aliyuncs.com/2022/08/28/f3938a0368c540ee909ba7f7079a829a.png)\n\n# 十大\n## 时代的",
    "createBy":"1",
    "createTime":"2022-08-28 15:15:46",
    "delFlag":0,
    "id":"10",
