...
List<Integer> list = Lsit.of(1, 2, 5);//不接受null
```
...
- 运行时数据区：java程序运行时数据的存放，比如对象，类信息，引用等等。
  ...
  - java栈
    ...
    - 
  - 本地方法栈
    - 而本地方法栈则为虚拟机使用到的 Native 方法服务。java代码可以通过本地方法接口调用OS底层的功能。那这些本地方法运行时使用到的内存就是本地方法栈
  - 程序计数器
...
```java
boolean match=Pattern.matches(resStr,content);//整体匹配，完全匹配返回true
...
Pattern pattern=Pattern.compile(regstr);//模式对象
Matcher matcher=pattern.matcher(content);//匹配器
while(matcher.find()){//能够找得到一个匹配的子串
    System.out.print(match.group(0));
}
match.start();//返回以前匹配的开始索引，或者说是当前匹配
match.start(int group);//返回开始索引
match.end();//返回以前匹配的结束索引+1
match.end(int group);//返回结束索引+1

String newString=matcher.replaceAll("hello");//将regStr匹配到的字符都替换为"hello",不修改原字符串

String strs[]="a b sc c   d   d".split("\\s+");
```

String中的正则表达式
```java
String newString ="hello 133".replaceAll("\\d+","happy");
boolean isMatch="hello 133".matches("^\\W");
```

## 反向引用
圆括号（分组）中的内容被捕获之后，可以在之后使用
内部反向引用`\\`分组号，外部`$`分组号
...
"(\\d)\\1"//连着两个相同的数字
"(\\d)\\1{4}"//连着5个相同的数字
"(\\d)(\\d)\\2\\1"//个位和千位相同，十位与百位相同的数字

String newString=matcher.replaceAll("$1");//外部用$分组号
...
## java8特性
Nashorn引擎，在jvm上面运行js
### lambda特性
```java
Runnable r1=new Runnable(){
    @Override
    public void run(){
        System.put.println("hwlloworld");
    }
};
r1.run();
...
int x=com1.compare(12,23);
//lambda 表达式
Comparator<Integer> com2=(o1,o2)->Integer.compare(o1,o2);//类型推断省略Integer
//方法引用
Comparator<Integer> com3= Integer::compare ;
Comsumer<String>son=(String s)->{System.out.println(s);};
Comsumer<String>son=s->{System.out.println(s);};//参数只有一个可以省略括号
```

`->`lambda操作符，箭头操作符
箭头左边 形参列表，接口抽象方法中的形参列表
箭头右边 重写的方法体

lambda的本质作为接口的实例（函数式接口的实例），匿名实现类都可以使用lambda来写
### 函数式（functional）接口特性
...
- 断定型接口`Predicate<T> boolean test()`
...
不能用对象调用静态方法
接口中的抽象方法的形参列表和返回类型和方法引用中的方法的形参列表和返回类型一致
> 当天要传递给lambda的方法体已经有实现的方法了可以使用方法引用
...
com1.compare(19,20);
Function<Double,Long>func=new Function(){
    @Override
    public void Long apply(Double d){
        return Math.round(d);
    }
};
...
//有两个参数，其中一个s1调用的方法，s2为该方法的形参

Function<Employee,String> fun1=Employee::getName;
```

构造器引用
接口中的抽象方法的形参列表和返回类型和构造器引用中的方法的形参列表和返回类型一致
...
Supplier<Employee> sup=Employee::new;//无参构造器
sup.get();
...
数组引用
数组可以看作是一个特殊的类构造器引用一样
```java
Function<Integer,String[]> funs=length->new String[length];
Function<Integer,String[]> funs=String[]::new;
String arr[]=funs.apply(12);
```
### 强大的`Stream`特性
`Stream`api `java.util.stream`
可以对集合数据进行操控，就像sql对数据库的操作一样
`Collections`是静态的数据结构，面向内存。`Stream`是计算用的面向cpu
stream操作时延迟执行的
操作`stream`的三个步骤
- 创建Stream
  - 一个数据源获取一个流
- 中间操作
  - 对数据源的数据进行操作
- 终止操作
  - 一旦执行终止操作，就会执行中间操作并产生结果且不会再次执行。（延迟执行的体现）

#### 创建
```java
List<Employee> employees=new LinkedList<>();
employees.add(new Employee("tom",132,"男"));//..
//1通过集合
Stream<Employee> stram=employees.stream();//顺序流
Stream<Employee> parallelstream=employee.parallelStream();//并行流
//2通过数组
int arr[]=new int[]{1,2,3,4,5,6,7,8,9};
IntStream stream=Arrays.stream(arr);
//LongStream 
//DoubleStream
Employee []em1=new Employeee[10];
Stream<Employee> stream=Arrays.stream(em1);

//3通过streamd的of
Stream<Integer> stream=Stream.of(1,2,3,4,5,6,7,8);
//4 创建无限流
//迭代
Stream.iterate(int seed,t->t+2).limit(10).forEach(System.out::println);
//生成
...
limit(int n);//截断流，使其元素不超过指定容量
skip(int n);//跳过元素，跳过前n个元素
distinct();//筛选通过流生成的hashCode和equals方法去除重复元素
...
stream.filter(e->e.getSalary()>7000).forEach(System.out::println);//遍历
stream.distanct().forEach(System.out::println);//去除重复元素
```
##### 映射
```java
map()//接受一个函数作为参数，将元素转化为其他形式或者提取信息
List<String> list;
list.stream().map(str-> str.toUpperCase).forEach(System.out::println);

flatmap();//接受一个函数作为参数，将流中的每个值都转化为另一个流，将所有的流连成一个流

Stream<Character> newstream=list.stream.flatmap(Employee::fun);
```
##### 排序
```java
sorted();//产生一个新流其中按照自然顺序排序
sorted(Comparator com);//产生一个新流其中比较器顺序排序
```
...
Optional<T> findFirst();//返回匹配的第一个元素
Optional<T> findAny();//返回匹配的任意一个元素
long count();//流中元素个数
Optional<T> max(Comparator);//最大值
Optional<T> min(Comparator);//最小值
forEach();//内部迭代
```
...
可以将流中的元素串联起来得到一个值返回
```java
Integer sum = list.stream().reduce(int initalzValue,Integer::sum);//求和
Stream<Double> stream=employees.stream().map(Employees::getSalary);
Optional<Double>sum=stream.reduce(Double::sum);
Optional<Double>sum=stream.reduce((d1,d2)->d1+d2);
```
##### 收集 
`collect(Collector c)`Collector接口方法中定义了如何对流执行收集的操作入收集到`List,Set,Map`中

Collectors类实现了Collector接口
```java
List<T>newlist=list.stream().collect(Collectors.toList())
Set<T>newset=list.stream().collect(Collectors.toSet())
```

### `Opeional`类
`Optional<T>`  `java.util.Optional`是一个容器类，可以保存类型T的值,代表值存在或者保存null，表示值不存在。
...
```java
Girl gril=new Gril();
Optional<Gril> go = Optional.of(girl);//不能为null

Optional<Gril> go = Optional.ofNullable(girl);
Optional<Gril> go = Optional.ofNullable(null);
go.orElse(new Girl("张张"));如果go中封装的对象为null则返回orElse中的对象

//对象包含值，例如Double，Long类型。明确知道不为空的情况下字何解获取值
T get();

//判断是否包含数据
boolean isPresent();
```
of--get    ofNullable--orElse
...
对于普通的共享变量来讲，线程A将其修改为某个值发生在线程A的本地内存中，此时还未同步到主内存中去；而线程B已经缓存了该变量的旧值，所以就导致了共享变量值的不一致。解决这种共享变量在多线程模型中的不可见性问题，较粗暴的方式自然就是加锁，但是此处使用synchronized或者Lock这些方式太重量级了，比较合理的方式其实就是`volatile`。
...
b.在进行指令优化时，不能将在对`volatile`变量访问的语句放在其后面执行，也不能把volatile变量后面的语句放到其前面执行。
...
## 代理模式
>使用一个代理将对象包装起来，然后用该代理对象取代原始对象。任何对对象的调用都需要通过代理，可以增强功能并且不修改原有代码
...
- 静态代理
  - 可能会需要太多的代理类，实现的结构相似
- 动态代理
  - 程序运行的时候，动态的创建代理类，使用反射
  - 根据加载到内存的被代理类创建一个代理类及其对象
  - 通过代理类的对象调用方法的时候动态的去调用被代理类中的同名方法
